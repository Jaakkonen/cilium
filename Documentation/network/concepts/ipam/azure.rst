...
.. note::
   ...
   * `Bring your own CNI <https://learn.microsoft.com/en-us/azure/aks/use-byo-cni?tabs=azure-cli>`__
   ...
   running on Azure VMs or VMSSs and AKS clusters on BYOCNI mode.
...
auto-detected from the `Azure Instance Metadata Service (IMS)
...
Subscription (``--azure-subscription-id``)
   ...
   operator detects the subscription of the node it runs on via its IMS.
...
Resource group (``--azure-resource-group``)
   ...
   on via its IMS, which is only correct when the operator pod runs on a node in
...
Node resource group
   ...
   from IMS when the operator is colocated with the nodes).
